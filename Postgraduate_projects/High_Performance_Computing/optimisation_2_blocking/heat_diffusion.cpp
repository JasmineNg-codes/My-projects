...
///
/// \section opt1_title Optimisation 2: Cache Blocking
///
...
///
/// This particular documentation is for \b `optimisation_2_blocking`.
///
...
private:
    ...
    std::vector<std::vector<double>> temperature;    ///< 2D temperature grid (restored from flattened)
    ...
    void saveFrame(int frameNumber) {
        system("mkdir -p output"); // Ensures the output directory exists
...
public:
    ...
    HeatDiffusion(int w, int h, double rate) 
        ...
        // temperature and nextTemperature are 2D vectors (height × width), initialized with 20°C
        ...
        // Initial condition: Setting a 7x7 hot spot (100°C) at the center of the grid
        for (int y = centerY - 3; y <= centerY + 3; y++) {
            for (int x = centerX - 3; x <= centerX + 3; x++) {
                if (y >= 0 && y < height && x >= 0 && x < width) {
                    temperature[y][x] = 100.0;  // 2D vector access
                }
            }
        }
    }
     ...
     * - The 2D grid is processed in **blocks of size `block_size × block_size`**, improving cache reuse.
     * - The block boundaries are carefully managed to stay within valid simulation indices (i.e., skipping edges).
     */
    void update() {
        static int frameCount = 0;
        const int block_size = 69;  // You can tune this (8, 16, 32 are common)

        // initially, whole row is 100, but blocking is 16 by 16
        // better for preloading up and down neighbours.
        // trying 16

        for (int y_block = 1; y_block < height - 1; y_block += block_size) {
            for (int x_block = 1; x_block < width - 1; x_block += block_size) {

                int y_limit = std::min(y_block + block_size, height - 1);
                int x_limit = std::min(x_block + block_size, width - 1);

                for (int y = y_block; y < y_limit; y++) {
                    for (int x = x_block; x < x_limit; x++) {
