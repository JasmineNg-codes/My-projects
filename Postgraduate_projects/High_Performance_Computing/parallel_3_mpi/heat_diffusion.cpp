...
///
/// \section opt1_title Parallelisation 4: MPI
///
...
///
/// This particular documentation is for \b `parallel_4_mpi`.
///
...
/**
 * @brief Saves the global temperature grid to a text file
 *
 * This function is called only by rank 0 and writes the reconstructed full
 * temperature field to a file in the `output` directory.
 *
 * @param fullGrid Flattened global temperature grid
 * @param frame Frame index for the output filename
 ...
 * @brief Start point for the MPI-based 2D heat diffusion simulation
 *
 * Initializes MPI, sets up a 2D Cartesian topology, and divides the domain among processes.
 * Handles halo exchange, local grid updates, and gathers the full solution every 10 steps.
 *
 * **MPI Features**:
 * - 2D Cartesian communicator (`MPI_Cart_create`)
 * - Halo exchange using `MPI_Sendrecv`
 * - Gather at rank 0
 *
 * This simulation runs multiple times as configured by the command line.
 */
int main(int argc, char** argv) {
    ...
    for (int run = 0; run < runs; ++run) {
        ...
        for (int step = 0; step < steps; ++step) {
            ...
            // Update local interior cells (no cache blocking)
