...
og_im = "./data_4/t2/images" #Just t2 substrate, change this path to desktop path to images 
og_lp = "./data_4/t2/annotations"

LPPreprocess(impath = og_im, lppath = og_lp, validation = True, train_ratio = 0.8, seed=100)

train_impath='./data_4/t2/train_images'
train_lppath='./data_4/t2/train_annotations'

val_impath='./data_4/t2/val_images'
val_lppath='./data_4/t2/val_annotations'
...
results_dir = os.path.join(script_dir, "..", "results","morph_t2")  # correct relative path
results_dir = os.path.abspath(results_dir)  # convert to absolute path
os.makedirs(results_dir, exist_ok=True)     # ensure it exists
df_results.to_csv(os.path.join(results_dir, "morph_t2.csv"), index=False)
print("Saved to:", os.path.join(results_dir, "morph_t2.csv"))
