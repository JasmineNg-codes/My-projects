...
results_dir = os.path.join(project_root, "results", "radius_t2") #store it under baseline_t1 results
kmeans_dir = os.path.join(results_dir, "kmeans_plots")
os.makedirs(kmeans_dir, exist_ok=True)

og_im = "./data_5/t2/images" #Just t2 substrate, change this path to desktop path to images 
og_lp = "./data_5/t2/annotations"

LPPreprocess(impath = og_im, lppath = og_lp, validation = True, train_ratio = 0.8, seed=100)

train_impath='./data_5/t2/train_images'
train_lppath='./data_5/t2/train_annotations'

val_impath='./data_5/t2/val_images'
val_lppath='./data_5/t2/val_annotations'
...
for i in lpcolor:
    ...
    for j in backgroundcolor: 
        ...
        plot_path = os.path.join(kmeans_dir, f"t2_kmean_lp{i}_bkgd{j}.html")
...
results_dir = os.path.join(script_dir, "..", "results","radius_t2")  # correct relative path
results_dir = os.path.abspath(results_dir)  # convert to absolute path
os.makedirs(results_dir, exist_ok=True)     # ensure it exists
df_results.to_csv(os.path.join(results_dir, "radius_t2.csv"), index=False)
print("Saved to:", os.path.join(results_dir, "radius_t2.csv"))