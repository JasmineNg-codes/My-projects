...
preprocessor = LPPreprocess(
    impath=os.path.join(project_root, "data_3/t2/images"),
    lppath=os.path.join(project_root, "data_3/t2/annotations"),
...
for fold, (train_idx, test_idx) in enumerate(kf.split(all_pairs)):
    ...
    temp_dir = os.path.join(project_root, "data_3/t2/cv_split")
...
results_dir = os.path.join(project_root, "results","kfold_t2")
os.makedirs(results_dir, exist_ok=True)
df_results = pd.DataFrame(results)
df_results.to_csv(os.path.join(results_dir, "kfold_t2.csv"), index=False)

print("Saved to:", os.path.join(results_dir, "kfold_t2.csv"))
